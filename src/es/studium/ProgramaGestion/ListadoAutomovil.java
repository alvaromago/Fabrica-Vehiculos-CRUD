	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource().equals(btnPdf))
		{
			...
			try
			{
				...
				table.addHeaderCell(new Cell().add(new Paragraph("Modelo").setFont(fontHeader).setBold()));
				table.addHeaderCell(new Cell().add(new Paragraph("Precio (€)").setFont(fontHeader).setBold()));
				table.addHeaderCell(new Cell().add(new Paragraph("Potencia (CV)").setFont(fontHeader).setBold()));

				// Agregar datos a la tabla desde el TextArea
				String[] lines = txaAutomovil.getText().split("\n");
